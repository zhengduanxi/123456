...
{
    printf(ANSI_FMT("device read memory: ", ANSI_FG_BLUE) FMT_PADDR ", the len is %d, the read data is " FMT_WORD ", the read device is "
                    ANSI_FMT(" %s ", ANSI_BG_BLUE) "\n", addr, len, data, map->name);
}

void display_device_write(uint32_t addr, int len, uint32_t data, IOMap *map)
{
    printf(ANSI_FMT("device write memory: ", ANSI_FG_YELLOW) FMT_PADDR ", the len is %d, the written data is " FMT_WORD ", the written device is "
                    ANSI_FMT(" %s ", ANSI_BG_YELLOW)"\n", addr, len, data, map->name);
}