...
void parse_elf(const char *elf_file)//const char *elf_file 参数表示要解析的 ELF 文件路径
{
    ...
    for(int i = 0; i < edhr.e_shnum; i++)//e_shnum: 节头表中的条目数量，某些文件可能不包含节头表，此时这个值是 SHN_UNDEF
    {
        ...
        {
            //获取字符串表
            string_table = malloc(shdr.sh_size);//如果找到字符串表，分配内存并读取字符串表的内容
    ...
    for(int i = 0; i < edhr.e_shnum; i++)
    {
        ...
        if(shdr.sh_type == SHT_SYMTAB)//循环遍历所有的节头，寻找符号表（SHT_SYMTAB 类型）
        {
            ...
            symbol = malloc(sizeof(Symbol) * sym_count);//如果找到符号表，分配内存并读取符号表的内容
...
{
    printf(ANSI_FMT("device read memory: ", ANSI_FG_BLUE) FMT_PADDR ", the len is %d, the read device is " 
                    ANSI_FMT(" %s ", ANSI_BG_BLUE) "\n", addr, len, map->name);
}

void display_device_write(paddr_t addr, int len, word_t data, IOMap *map)
{
    printf(ANSI_FMT("device write memory: ", ANSI_FG_YELLOW) FMT_PADDR ", the len is %d, the written data is " FMT_WORD 
                    ", the written device is "ANSI_FMT(" %s ", ANSI_BG_YELLOW)"\n", addr, len, data, map->name);
}

// etrace
void display_ecall(word_t pc, word_t npc) {
    printf("ecall at 0x%08x  to 0x%08x\n", pc, npc);
    fflush(stdout);
}

void display_mret(word_t pc, word_t npc) {
    printf("mret at 0x%08x  to 0x%08x\n", pc, npc);
    fflush(stdout);
}
